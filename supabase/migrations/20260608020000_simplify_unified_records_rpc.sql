...
-- Replace the UNION-based admin_list_unified_records with a catalog_items-only query.
-- Member profiles are now catalog_items with item_type = 'member' and linked_user_id IS NOT NULL.
-- The function signature is unchanged so no frontend changes are needed.
...
begin
  ...
  with catalog_rows as (
    select
      ci.id,
      -- items with a linked user are member profiles; all others are catalog items
