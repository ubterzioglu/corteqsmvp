...
alter table public.resource_entries
  ...
  add column if not exists is_public_import boolean,
  add column if not exists import_suggestion text,
...
alter table public.resource_entries
  drop constraint if exists resource_entries_department_check;

create index if not exists idx_resource_entries_section
  on public.resource_entries (section);

create index if not exists idx_resource_entries_section_subsection
  on public.resource_entries (section, subsection);

create index if not exists idx_resource_entries_order_no
  on public.resource_entries (order_no);
