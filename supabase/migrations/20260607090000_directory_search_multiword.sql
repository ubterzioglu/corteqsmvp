-- Improve search_directory_catalog to support multi-word queries.
-- "Doktor Dortmund" now matches a doctor located in Dortmund because each
-- word is tested independently against all searchable fields, and ALL words
-- must match (AND semantics). Single-word searches behave identically to before.
-- Also added: role label (r.label / 'Doktor', 'Avukat' etc.) to search scope.
...
begin
  ...
  -- regexp_split_to_array with \s+ handles multiple spaces gracefully.
  ...
  where ipd.visibility_status = 'open'
    ...
    and (
      ...
      or (
        select bool_and(
          coalesce(p.display_name, '') ilike '%' || word || '%'
...
-- Re-grant execute (function signature unchanged, but explicit grant ensures correctness).
