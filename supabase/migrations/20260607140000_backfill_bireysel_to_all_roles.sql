...
-- Re-apply bireysel baseline to ALL currently active roles (including roles
-- added after the 20260605100000 migration). Fully idempotent — safe to run
-- multiple times. Does not downgrade any role-specific extras already set.
...
-- 2. Upsert role_feature_defaults for all active roles
with source_role as (
  select id, key
  from public.roles
  where key = 'bireysel'
...
-- 3. Upsert role_feature_flags for all active roles
with source_role as (
  select id, key
  from public.roles
  where key = 'bireysel'
...
-- 4. Upsert role_attribute_rules for all active roles
with source_role as (
  select id
  from public.roles
  where key = 'bireysel'
...
-- 5. Upsert role_profile_section_rules for all active roles
with source_role as (
  select id
  from public.roles
  where key = 'bireysel'
