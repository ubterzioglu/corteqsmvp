...
-- Apply bireysel baseline rules to Healthcare_Doctor role.
-- Identical logic to 20260607140000_backfill_bireysel_to_all_roles.sql but
-- scoped to a single target role. Idempotent — safe to run multiple times.

-- 1. Widen feature_catalog scope: bireysel-scoped features → global
with source_role as (
  select id, key
  from public.roles
  where key = 'bireysel'
...
-- 2. Upsert role_feature_defaults for Healthcare_Doctor
with source_role as (
  select id, key
  from public.roles
  where key = 'bireysel'
...
insert into public.role_feature_defaults (
  profile_type,
  feature_key,
  is_enabled
)
select
  ...
  true
from target_role tr
cross join source_features sf
on conflict (profile_type, feature_key) do update
set is_enabled = excluded.is_enabled;

-- 3. Upsert role_feature_flags for Healthcare_Doctor
with source_role as (
  select id, key
  from public.roles
  where key = 'bireysel'
...
insert into public.role_feature_flags (
  role_id,
  feature_key,
  is_enabled,
  updated_by
...
-- 4. Upsert role_attribute_rules for Healthcare_Doctor
with source_role as (
  select id
  from public.roles
  where key = 'bireysel'
...
select
  ...
  sar.sort_order
from target_role tr
cross join source_attribute_rules sar
...
-- 5. Upsert role_profile_section_rules for Healthcare_Doctor
with source_role as (
  select id
  from public.roles
  where key = 'bireysel'
...
select
  ...
  ssr.sort_order
from target_role tr
cross join source_section_rules ssr
