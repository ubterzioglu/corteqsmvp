...
alter table public.turkish_missions enable row level security;
alter table public.turkish_mission_units enable row level security;
alter table public.turkish_mission_relations enable row level security;

drop policy if exists "Public can read active turkish missions" on public.turkish_missions;
create policy "Public can read active turkish missions"
on public.turkish_missions
for select
to anon, authenticated
using (status = 'active');

drop policy if exists "Public can read turkish mission units" on public.turkish_mission_units;
create policy "Public can read turkish mission units"
on public.turkish_mission_units
for select
to anon, authenticated
using (
  exists (
    ...
    where tm.slug = turkish_mission_units.mission_slug
      and tm.status = 'active'
  )
);

drop policy if exists "Public can read turkish mission relations" on public.turkish_mission_relations;
create policy "Public can read turkish mission relations"
on public.turkish_mission_relations
for select
to anon, authenticated
using (
  exists (
    select 1
    from public.turkish_missions tm
    where tm.slug = turkish_mission_relations.parent_mission_slug
      and tm.status = 'active'
  )
  and exists (
    select 1
    from public.turkish_missions tm
    where tm.slug = turkish_mission_relations.child_mission_slug
      and tm.status = 'active'
  )
);

drop policy if exists "Admins can manage turkish missions" on public.turkish_missions;
create policy "Admins can manage turkish missions"
on public.turkish_missions
for all
to authenticated
...
drop policy if exists "Admins can manage turkish mission units" on public.turkish_mission_units;
create policy "Admins can manage turkish mission units"
on public.turkish_mission_units
for all
to authenticated
using (public.is_admin(auth.uid()))
...
drop policy if exists "Admins can manage turkish mission relations" on public.turkish_mission_relations;
create policy "Admins can manage turkish mission relations"
on public.turkish_mission_relations
for all
to authenticated
using (public.is_admin(auth.uid()))
...
grant select on public.turkish_missions to anon, authenticated;
grant select on public.turkish_mission_units to anon, authenticated;
grant select on public.turkish_mission_relations to anon, authenticated;
grant insert, update, delete on public.turkish_missions to authenticated;
grant insert, update, delete on public.turkish_mission_units to authenticated;
grant insert, update, delete on public.turkish_mission_relations to authenticated;
