...
-- Rewrite get_catalog_item_profile to resolve attribute and feature rules from
-- role_attribute_rules / role_feature_flags when the item's catalog_item_type
-- has a linked_role_key. Falls back to the old item_type_attribute_rules /
-- item_type_feature_defaults path for item types that have no linked role yet.
...
begin
  ...
  -- Resolve linked role for this item type
  select r.id
  into v_role_id
...
-- Grants unchanged — security definer, authenticated already has execute
