...
CREATE TABLE public.may19_submissions (
...
CREATE INDEX idx_may19_kind ON public.may19_submissions(kind);
CREATE INDEX idx_may19_created ON public.may19_submissions(created_at DESC);
CREATE INDEX idx_may19_status ON public.may19_submissions(status);

ALTER TABLE public.may19_submissions ENABLE ROW LEVEL SECURITY;

CREATE POLICY "anyone can submit may19"
  ON public.may19_submissions FOR INSERT
  TO anon, authenticated
  WITH CHECK (true);

CREATE POLICY "approved map pins are public"
  ON public.may19_submissions FOR SELECT
  TO anon, authenticated
...
CREATE POLICY "admins update may19"
  ON public.may19_submissions FOR UPDATE
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "admins delete may19"
  ON public.may19_submissions FOR DELETE
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));
