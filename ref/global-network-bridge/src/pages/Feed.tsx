...
import CafeAmbiance from "@/components/feed/CafeAmbiance";
import { getUserInterests, sortByInterests } from "@/lib/interestTargeting";
...
const Feed = () => {
  ...
  const fetchPosts = useCallback(
    async (reset = false) => {
      ...
      // Interest-based targeting: bireysel kullanıcının seçtiği ilgi alanlarına
      // uyan postlar yukarı çıkar (sayfa içi yeniden sıralama).
      const interests = getUserInterests(user?.id);
      const ranked = sortByInterests(newPosts, interests, (p) => p.content);
      setPosts((prev) => (reset ? ranked : [...prev, ...ranked]));
              ...
              {!inCafe && (
                <div className="mb-4">
                  ...
                  <div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1 snap-x">
                    <Link to={user ? categoryAccountLink : "/auth"} className="shrink-0 w-20 snap-start flex flex-col items-center gap-1 group">
                      ...
                      <div className="text-[10px] font-semibold text-center text-primary">Cafe'ni Ekle</div>
                    </Link>
                    {/* Demo cafe story */}
              ...
              {inCafe ? (
                <>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <h1 className="text-xl font-extrabold truncate flex items-center gap-2 flex-wrap">
                          ...
                          {cafe!.kind === "community" && cafe!.open_entry && <Badge className="bg-emerald-500/15 text-emerald-600 border-0 text-[10px]">🟢 Giriş Serbest</Badge>}
                          {cafe!.kind === "community" && !cafe!.open_entry && <Badge variant="outline" className="text-[10px] border-amber-500/40 text-amber-700 dark:text-amber-300"><Lock className="h-2.5 w-2.5 mr-1" />Koşullu Giriş</Badge>}
                        ...
                        {/* Cafe nüfus barı */}
                        {(() => {
                          const cap = cafe!.capacity || 0;
                          const cur = cafe!.member_count || 0;
                          const max = cap > 0 ? cap : Math.max(50, cur + 10);
                            ...
                            <div className="mt-1.5 max-w-[220px]">
                              <div className="flex items-center justify-between text-[9px] font-semibold mb-0.5">
                                <span className="flex items-center gap-1 text-foreground">
                                  ...
                                  <span className="text-muted-foreground font-normal">/ max {cap > 0 ? cap : "∞"}</span>
                                </span>
                                {full ? (
                                  <span className="text-rose-600">DOLU</span>
                                ) : (
                              ...
                              <div className="h-1 w-full rounded-full bg-muted overflow-hidden">
                                <div
                                  className={`h-full transition-all duration-500 ${full ? "bg-rose-500" : pct > 75 ? "bg-amber-500" : "bg-emerald-500"}`}
                                  style={{ width: `${pct}%` }}
                                />
                              </div>
                            </div>
                  ...
                  <CafeAmbiance
                    cafeId={cafeId!}
                    cafeName={cafe!.name}
                    isOwner={!!user && cafe!.created_by === user.id}
                    isMember={isMember && approved}
                  />
                </>
