...
import { setRequestVisible } from "@/lib/serviceRequestVisibility";
...
const ServiceRequestForm = ({ onSuccess, onCancel }: ServiceRequestFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (inserted?.id) setRequestVisible(inserted.id, showOnProfile);
      ...
      <div className="flex items-start gap-3 rounded-xl border border-border bg-muted/30 p-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-3">
            <Label htmlFor="show-on-profile" className="text-sm font-semibold cursor-pointer">
              Talebimi profilimde göster
            </Label>
            <Switch id="show-on-profile" checked={showOnProfile} onCheckedChange={setShowOnProfile} />
          </div>
          <p className="text-[11px] text-muted-foreground mt-1">
            Talebiniz Profilinizde Görünür — diğer üyeler profil sayfanızdaki "Hizmet Talepleri" sekmesinden bu talebi görebilir. İstediğiniz zaman kapatabilirsiniz.
          </p>
        </div>
      </div>
