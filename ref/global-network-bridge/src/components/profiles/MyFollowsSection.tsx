...
import { Heart, Star, Users, Briefcase, PenLine, Calendar, X, Eye, EyeOff } from "lucide-react";
...
const VISIBILITY_KEY = "corteqs:show_follows_on_profile";
const MAX_FOLLOWS_ON_PROFILE = 50;

export const isFollowsVisibleOnProfile = () => {
  try {
...
const MyFollowsSection = () => {
  ...
  const [visible, setVisible] = useState<boolean>(() => isFollowsVisibleOnProfile());

  useEffect(() => {
    try { localStorage.setItem(VISIBILITY_KEY, String(visible)); } catch {}
    window.dispatchEvent(new CustomEvent("corteqs:follows-visibility-change"));
  }, [visible]);
  ...
  return (
    <div className="bg-card rounded-2xl border border-border p-6 shadow-card">
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Takip ettiğiniz danışman, kuruluş, işletme, blogger ve etkinlikler burada listelenir. Profilinizde en fazla {MAX_FOLLOWS_ON_PROFILE} kişi görünür.
      </p>

      <div className="mb-4 flex items-center justify-between gap-3 rounded-xl border border-border bg-muted/30 p-3">
        <div className="flex items-center gap-2 min-w-0">
          {visible ? <Eye className="h-4 w-4 text-success shrink-0" /> : <EyeOff className="h-4 w-4 text-muted-foreground shrink-0" />}
          <div className="min-w-0">
            <Label htmlFor="follows-visibility" className="text-sm font-medium cursor-pointer">
              Takip ettiklerimi profilimde göster
            </Label>
            <p className="text-[11px] text-muted-foreground">
              {visible ? "Profilini ziyaret edenler listeyi görebilir." : "Liste yalnızca sana özel — kimse göremez."}
            </p>
          </div>
        </div>
        <Switch id="follows-visibility" checked={visible} onCheckedChange={setVisible} />
      </div>
