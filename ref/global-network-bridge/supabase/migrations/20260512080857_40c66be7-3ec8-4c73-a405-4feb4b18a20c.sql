...
-- Generic helper: notify followers of a given author with given content
...
BEGIN
  ...
  WHERE uf.following_id = _author_id;
END;
$$;

-- Events: notify followers when a published event is created
...
BEGIN
  IF NEW.status <> 'published' THEN
    RETURN NEW;
  END IF;
  SELECT COALESCE(full_name, 'Takip ettiğin kişi') INTO v_author_name
  ...
  PERFORM public.notify_followers(
    ...
    v_author_name || ' yeni bir etkinlik yayınladı (' ||
      COALESCE(NEW.city, NEW.country, 'Online') || ', ' || NEW.event_date::text || ').',
    NEW.id
  );
  RETURN NEW;
END;
...
-- Job listings: notify followers when a published listing is created
...
BEGIN
  IF NEW.status <> 'published' THEN
    RETURN NEW;
  END IF;
  SELECT COALESCE(NEW.business_name, full_name, 'Takip ettiğin işletme') INTO v_author_name
  FROM public.profiles WHERE id = NEW.user_id;
  ...
  PERFORM public.notify_followers(
    ...
    v_author_name || ' yeni bir ' || COALESCE(NEW.employment_type,'iş') || ' ilanı yayınladı' ||
...
FOR EACH ROW EXECUTE FUNCTION public.notify_followers_on_job();

-- Cafes: notify followers when a new cafe is created
...
BEGIN
  ...
  PERFORM public.notify_followers(
    ...
    v_author_name || ' "' || NEW.name || '" cafe''sini açtı (' ||
      COALESCE(NEW.city || ' · ', '') || COALESCE(NEW.theme, NEW.kind) || ').',
    NEW.id
  );
  RETURN NEW;
END;
...
FOR EACH ROW EXECUTE FUNCTION public.notify_followers_on_cafe();
