  ...
  <EmojiPicker.Root onEmojiSelect={({ emoji }) => onSelect(emoji)} columns={8} className="bg-white">
    <div className="border-b border-slate-100 p-2">
      <EmojiPicker.Search
        ...
        className="h-9 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 text-sm outline-none transition placeholder:text-slate-500 focus:border-orange-300 focus:bg-white focus:ring-2 focus:ring-orange-100"
      />
    </div>
      ...
      <EmojiPicker.List
        components={{
          CategoryHeader: ({ category, className, ...props }) => (
            <div {...props} className={cn("sticky top-0 z-10 bg-white/95 px-1 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-slate-500", className)}>
              {category.label}
            </div>
          ),
          Row: ({ className, ...props }) => (
